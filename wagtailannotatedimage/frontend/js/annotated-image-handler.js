class AnnotatedImageEditHandler {
	...
	attachObserver() {
		...
		).observe(this.container[0].querySelector("#" + this.imageFieldId), {
			attributes: true
		});
	}
	...
	createAnnotationForm(index, initialData) {
		var annotationForm = this.container
			.find("[data-annotation-form]")
			.clone(false);
		annotationForm.removeAttr("data-annotation-form");
		annotationForm
			.find('[name="' + this.formPrefix + 'annotation_number"]')
			.val(index);
		...
		annotationForm
			.find("button")
			.click(this.deleteAnnotationHandler.bind(this));
		if (initialData && Object.keys(initialData.fields).length > 0) {
			var fields = initialData.fields;
			for (name in fields) {
				//input
				var input = annotationForm.find(
					'[name="' + this.formPrefix + name + '"]'
				);
				if (input.length > 0) {
		...
		annotationForm
			...
			.change(this.annotationFieldHandler.bind(this));
	...
	annotationFieldHandler(event) {
		...
		var id = parent.querySelector(
			'[name="' + this.formPrefix + 'annotation_number"]'
		).value;
		var annotations = this.annotationData;
